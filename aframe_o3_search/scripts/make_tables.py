...
def main(
    ...
    columns = ["event", "catalog", "aframe_far", "far", "aframe_p_astro", "p_astro", "mass_1", "mass_2"]
    ...
    recovered = recovered.assign(aframe_p_astro=p_astro(recovered.aframe_detection_statistic.values))
    ...
    analyzed.loc[:, "event"] = analyzed.event.apply(lambda x: x.replace("_", "\_"))
    ...
    found['mass_1'] = found.apply(lambda row: mass_formatter(row, "mass_1"), axis=1)
    found['mass_2'] = found.apply(lambda row: mass_formatter(row, "mass_2"), axis=1)
    found["catalog"] = "GWTC-3"
    table = found[columns]

    mask = table.aframe_p_astro >= 0.5
    significant = pd.concat([significant, table[mask]], ignore_index=True)
    non_significant = pd.concat([non_significant, table[~mask]], ignore_index=True)

    # ogc 
    logger.info("Building OGC catalog")

    # read in the new candidates reported by OGC
    ogc: pd.DataFrame = Table.read(pipeline_data_dir / "ogc" / "new.tex", format="latex").to_pandas()
    ...
    ogc["catalog"] = "OGC"
    ogc_columns = ["event", "catalog", "aframe_far", "far", "aframe_p_astro", "p_astro", "m_1", "m_2"]
    ogc = ogc[ogc_columns]
    ogc.rename(columns = {ogc: col for ogc, col in zip(ogc_columns, columns)}, inplace=True)

    mask = ogc.aframe_p_astro >= 0.5 
    significant = pd.concat([significant, ogc[mask]], ignore_index=True,)
    non_significant = pd.concat([non_significant, ogc[~mask]], ignore_index=True)
    ...
    ares: pd.DataFrame = Table.read(pipeline_data_dir / "aresgw" / "new.tex", format="latex").to_pandas()
    ...
    ares["catalog"] = "AresGW"
    ares_columns = ["event", "catalog", "aframe_far", "far", "aframe_p_astro", "p_astro", "m_1", "m_2"]
    ares = ares[ares_columns]
    ares.rename(columns={ares: col for ares, col in zip(ares_columns, columns)}, inplace=True)

    mask = ares.aframe_p_astro > 0.5     
    significant = pd.concat([significant, ares[mask]], ignore_index=True)
    non_significant = pd.concat([non_significant, ares[~mask]], ignore_index=True)

    # IAS
    logger.info("Building IAS catalog")
    # read in the new candidates reported by IAS
    # from o3a and b and combine them
    ias_o3a: pd.DataFrame = Table.read(pipeline_data_dir / "ias" / "new_o3a.latex", format="latex").to_pandas()
    ...
    ias["catalog"] = "IAS" 
    ias_columns = ["event", "catalog", "aframe_far", "far", "aframe_p_astro", "p_astro", "m_1", "m_2"]
    ias = ias[ias_columns]
    ias.rename(columns={ias: col for ias, col in zip(ias_columns, columns)}, inplace=True) 
    mask = ias.aframe_p_astro >= 0.5
    significant = pd.concat([significant, ias[mask]], ignore_index=True)
    non_significant = pd.concat([non_significant, ias[~mask]], ignore_index=True)
    
    # now reset index
    significant = significant.sort_values("aframe_p_astro", ascending=False)
    non_significant = non_significant.sort_values("aframe_p_astro", ascending=False)
    ...
    print(significant.aframe_p_astro)
    latex_sig = significant.to_latex(
        columns=columns,
        header = ["Event", "Catalog", "Aframe FAR $\mathrm{{yr}}^{{-1}}$", "FAR $\mathrm{{yr}}^{{-1}}$", "Aframe $p_\mathrm{{astro}}$", "$p_\mathrm{{astro}}$", "$m_1 (M_{{\odot}})$", "$m_2 (M_{{\odot}})$"],
        formatters = {
    ...
    sig_caption = "Candidates that Aframe detects with $p_\mathrm{astro} \geq 0.5$. The catalog each candidate was originally reported in is listed. Events marked with an asterisk occurred during the training period."
    sig_label = "table:significant" 
    latex_sig = build_table(latex_sig, sig_caption, sig_label, len(columns))
    non_sig_caption = "Candidates that Aframe detects with $p_\mathrm{astro} < 0.5$. The catalog each candidate was originally reported in is listed. Events marked with an asterisk occurred during the training period."
